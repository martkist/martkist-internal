...
{
    return QCoreApplication::translate("martkist-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. martkist_de.qm (shortcut "de" needs to be defined in martkist.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. martkist_de_DE.qm (shortcut "de_DE" needs to be defined in martkist.qrc)
...
/** Class encapsulating Martkist Core startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class MartkistCore: public QObject
{
    Q_OBJECT
public:
    explicit MartkistCore();
...
/** Main Martkist application object */
class MartkistApplication: public QApplication
{
    Q_OBJECT
public:
    explicit MartkistApplication(int &argc, char **argv);
    ~MartkistApplication();
...
#include "martkist.moc"

MartkistCore::MartkistCore():
    QObject()
{
}

void MartkistCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(GetWarnings("gui")));
}

void MartkistCore::initialize()
{
    try
    {
...
void MartkistCore::restart(QStringList args)
{
...
MartkistApplication::MartkistApplication(int &argc, char **argv):
    ...
    returnValue(0)
{
    ...
    // This must be done inside the MartkistApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = GetArg("-uiplatform", MartkistGUI::DEFAULT_UIPLATFORM);
...
MartkistApplication::~MartkistApplication()
{
    if(coreThread)
    {
...
void MartkistApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(NULL, resetSettings);
}

void MartkistApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MartkistGUI(platformStyle, networkStyle, 0);
...
void MartkistApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void MartkistApplication::startThread()
{
    if(coreThread)
        return;
    coreThread = new QThread(this);
    MartkistCore *executor = new MartkistCore();
...
void MartkistApplication::parameterSetup()
{
    InitLogging();
...
void MartkistApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        if(pwalletMain)
        {
            ...
            window->addWallet(MartkistGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MartkistGUI::DEFAULT_WALLET);
        ...
        // martkist: URIs or payment requests:
...
void MartkistApplication::shutdownResult(int retval)
{
    qDebug() << __func__ << ": Shutdown result: " << retval;
    quit(); // Exit main loop after shutdown finished
}

void MartkistApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception", MartkistGUI::tr("A fatal error occurred. Martkist Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId MartkistApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    /// 6. Determine availability of data directory and parse martkist.conf
    ...
    try {
        ReadConfigFile(GetArg("-conf", MARTKIST_CONF_FILENAME));
    } catch (const std::exception& e) {
        QMessageBox::critical(0, QObject::tr(PACKAGE_NAME),
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Martkist Core"),
    ...
    // martkist: links repeatedly have their payment requests routed to this process:
